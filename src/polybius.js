  ...
  function polybius(input, encode = true) {
    ...
    if (encode) {
      return input.split('').map((character) => {
        // using an object.value to access [] so A then lowercase will go find a in the {} obj. 52,51,
        return alphaEncode[character.toLowerCase()]
      }).join('')
    } else { 
      // any spaces we will replace them. & if the length is not even it will be false. 
      if ((input.replace(/\s/g,'').length %2) !== 0) return false
        // from 0-9 has to be 2 of them or a space then map them out individually
        return input.match(/[0-9]{2}|\s/g).map((number) => {
          return alphaDecode[number]
        }).join('')
    }
